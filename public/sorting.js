const getRandomArray = (itemsCount = 100) => {
    let numsArray = Array.apply(null, Array(itemsCount)).map((v, i) => i + 1);
    return numsArray;
};
...
const shakeArray = (a) => {
    ...
    for (let i = 0; i < aCopy.length; i++) {
        let randIndex = Math.floor(Math.random() * aCopy.length);
        swapArrayItems(i, randIndex, aCopy);
    }
